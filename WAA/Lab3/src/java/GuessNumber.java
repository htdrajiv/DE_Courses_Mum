    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(userAnswer==null || userAnswer.isEmpty()){
                session.setAttribute("outputMessage", "");
            }else {
                int uAnswer = Integer.parseInt(userAnswer);
                if(uAnswer==toCheck){
                    session.setAttribute("outputMessage",toCheck+" :: Correct, Congratulations!");
                }else if(uAnswer>toCheck){
                    session.setAttribute("outputMessage",toCheck+" :: Too high, try again!");
                }else{
                    session.setAttribute("outputMessage",toCheck+" :: Too low, try again!");
                }
            }
